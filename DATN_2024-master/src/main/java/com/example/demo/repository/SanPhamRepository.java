...
public interface SanPhamRepository extends JpaRepository<SanPham, String> {
        ...
        SanPham getSPByTen(@Param("tenSP")String ten);
        ...
        @Query("SELECT new com.example.demo.dto.sanpham.SanPhamOnlineResponse(" +
                        ...
                        "LEFT JOIN sp.listCTSP ctp " +
                        "LEFT JOIN ctp.anhCTSP ac " +
                        "WHERE CAST(ctp.gia AS double) = (" +
                        "   SELECT MIN(CAST(ctp2.gia AS double)) " +
                        ...
                        "AND (:searchText IS NULL OR sp.tenSP LIKE %:searchText%) " +
                        "AND (:danhMuc IS NULL OR sp.danhMuc.ten IN :danhMuc) " +
                        "AND EXISTS (SELECT 1 FROM LoHang lh WHERE lh.ctsp.id = ctp.id AND lh.soLuong > 0)")
        Page<SanPhamOnlineResponse> findSanPhamOnline(
